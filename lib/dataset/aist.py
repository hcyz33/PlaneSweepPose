...
class Aist(torch.utils.data.Dataset):
    def __init__(self, cfg, image_set,num_views, mode = Mode.Demo) -> None:
    ...
    def get_aist(self):
        #TODO: iterate all sequence
        # Now we only use one sequence to test
        process_aist = PreProcessAist()
        cam = process_aist.get_camera()
        k2d = process_aist.get_keypoints_2d()
        k3d = process_aist.get_keypoints_3d()
        # there are 9 cameras for one subjucet, but we only need three of them to test
        return k2d[:self.num_views],cam[:self.num_views],np.array(k3d)*10
    ...
    def __getitem__(self, idx):
        ...
        for view_id in views:
            k, pv, jv, pd, jd, m = self._get_single_view_item(frame_id, view_id)
    ...
    def _get_single_view_item(self, frame_id, idx):
        ...
        pred_pose2d = self.k2d[np.newaxis, idx,frame_id,:,:] # [view_numbers, frame_numbers, Nj_coco, 2+1]
